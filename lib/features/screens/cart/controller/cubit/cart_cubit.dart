...
class CartCubit extends Cubit<CartState> {
  ...
  void addToCart(ProductModel product, BuildContext context) {
    ...
    if (product.productType == ProductType.variable.toString() &&
        state.productVariationModel.id.isEmpty) {
    ...
    final selectedCartItem = convertToCartItem(product, state.productQuantityInCart);
  ...
  void updateAlreadyAddedProductCount(ProductModel product) {
    if (product.productType == ProductType.single.toString()) {
      ...
      if (variationId.isNotEmpty) {
        emit(state.copyWith(
          productQuantityInCart: getVariationQuantityInCart(product.id, variationId),
        ));
      } else {
  ...
  void saveCartItems() {
    final cartItemStrings = state.cartItems.map((item) => item.toJson()).toList();
  ...
  Future<void> loadCartItems() async {
    ...
    if (cartItemSaved != null && cartItemSaved.isNotEmpty) {
      ...
      final cartItems = decodedData.map((item) => CartItemModel.fromJson(item)).toList();
